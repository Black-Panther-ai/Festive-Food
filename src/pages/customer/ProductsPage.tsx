  ...
  useEffect(() => {
    const fetchCatalog = async () => {
      ...
      try {
        ...
        if (!prodRes.ok) {
          throw new Error(
            `Products API returned ${prodRes.status}`
          );
        }
        ...
        console.error(
          'Failed to load products from Render API:',
          err
        );
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <div
              key={n}
              className="h-80 bg-stone-100 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : products.length === 0 ? (
        ...
        <div>
          <div className="text-xs text-stone-500 mb-4 font-medium">
            ...
            festive items ready for demand pre-order
          </div>
